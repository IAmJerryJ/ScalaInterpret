...
class TokensTests extends FlatSpec with Matchers {
  ...
  //
  ///////////////////////////////////////////////////////////////////
  it should "handle -: (- ( + (+ 7 2) 2) (- 4 1))" in {
    //complex arithmetics, check equal to 8 
    assert(eval(LList(LSymbol("-"),
                      LList(LList(LSymbol("+"),
                      LList(LList(LSymbol("+"),
                                  LList(LNumber(7),
  ...
  it should "handle cdr: (cdr (quote (a b c d)))" in {
    //should get b,c,d if correct
    assert(eval(LList(LSymbol("cdr"),
                      LList(LList(LSymbol("quote"),
                                  LList(LList(LSymbol("a"),
                                              LList(LSymbol("b"),
                                        ...
                                        LSymbol("nil"))),
                            LSymbol("nil"))))
           == LList(LSymbol("b"),LList(LSymbol("c"),LList(LSymbol("d"),LSymbol("nil")))))
  }

  it should "handle if when condition true: (if t 5 4)" in {
    //if true, get 5. 
    assert(eval(LList(LSymbol("if"),
                      LList(LSymbol("t"),
                            LList(LNumber(5),
  ...
  it should "detect non-numbers" in {
    //sub should not handle LSymbol
    assert(sub(LSymbol(","), LSymbol("a")) == LSymbol("ERROR"))
  }

  "arithmetic" should "handle /: (/ 6 2)" in {
    //Divide 6 by 2
    assert(eval(LList(LSymbol("/"),
                      LList(LNumber(6),LList(LNumber(2),LSymbol("nil")))))
           == LNumber(3))
  }

  "PARSING: pattern match" should "handle simple list (Hi how are you)" in {
    //check if can handle list of strings
    assert(matchPat("(Hi how are you)") == List("(", "hi", "how", "are", "you", ")"))
  }
  ...
  "strToLObj" should "handle a name howard" in {
    assert(strToLObj("howard") == LSymbol("howard"))
  }

  "eeqq" should "give t if equal opreator '/'' " in {
    //True if equal
